...
tmin = double(path_array(3,:)); %Initial t (vector) for each segment
tmax = double(path_array(4,:)); %final t (vector) for each segment
...
X_curr = X_curr(1) + ball_dim(2)*cos(eval(subs(theta(1),t,t_curr+atan_tol))+pi/2); %Finds initial x,y coords of the center of mass
Y_curr = eval(subs(sy,t,t_curr));
Y_curr = Y_curr(1) + ball_dim(2)*sin(eval(subs(theta(1),t,t_curr+atan_tol))+pi/2);
...
theta_prev = eval(subs(theta(1),t,t_curr+atan_tol)); %used later for accuracy improvement for finding X_contact
...
while t_curr < tmax(1)
    ...
    t_curr = eval(solve(-X_contact + subs(sx(1),t,tvar))) %uses sx_ge(1) equation
...
while t_curr < tmax(2)
    ...
    t_curr = eval(solve(-X_contact + subs(sx(2),t,tvar))) %uses sx_ge(2) equation
    % 'snap' Y_curr to curve + add radius of ball to account for iteration error
...
while t_curr < tmax(3)
    ...
    t_curr_temp = eval(solve(-X_contact + subs(sx(3),t,tvar)));
    t_curr = abs(t_curr_temp(1))
...
while t_curr > tmin(5)
    ...
    % *** Calculate t_curr
    t_curr_temp = eval(solve((sx(5) - X_contact),t));
    %Above equation was found by solving the sx_ge(3) == X_contact equation for t 
    t_curr = abs(t_curr_temp(1))
...
while t_curr < tmax(6)
    ...
    t_curr_temp = solve(sx(6) == X_contact);
    t_curr = abs(t_curr_temp(1));
...
while X_curr < 0.889 % While ball position isn't at the finish position
    ...
    %Find t_curr using sx_ge(7) equation (simplified) and X_contact
    t_curr = X_contact - sx(7) 
